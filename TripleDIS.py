...
class TripleDIS(object):
    def __init__(self, key1, key2, key3):
        # TripleDIS sınıfının başlatıcı metodu
        # key1, key2 ve key3 değerleri sınıfa atanır
        self.blocksize = 64
        self.rounds = 20
    ...
    def _rotate_left(val, r_bits, max_bits):
        # Bir değeri belirli bir sayıda sola döndüren yardımcı metot
    ...
    def _rotate_right(val, r_bits, max_bits):
        # Bir değeri belirli bir sayıda sağa döndüren yardımcı metot
    ...
    def _expand_key(key, wordsize, rounds):
        # Anahtarı genişleten yardımcı metot
        ...
        def _align_key(key, align_val):
            # Anahtarı belirli bir değere uygun hale getiren yardımcı metot
        ...
        def _const(w):
            # Sabit değerleri belirleyen yardımcı metot
            if w == 16:
        ...
        @staticmethod
        def _extend_key(w, r):
            # Anahtar uzatma işlemini gerçekleştiren yardımcı metot
            P, Q = _const(w)
            S = [P]
            t = 2 * (r + 1)
            for i in range(1, t):
        ...
        def _mix(L, S, r, w, c):
            # Anahtarı karıştıran yardımcı metot
            t = 2 * (r + 1)
            m = max(c, t)
            A = B = i = j = 0
            for k in range(t * m):
    ...
    def _encrypt_block(data, expanded_key, blocksize, rounds):
        # Blok şifreleme işlemini gerçekleştiren yardımcı metot
        w = blocksize // 2
        b = blocksize // 8
        mod = 2 ** w
    ...
    def _decrypt_block(data, expanded_key, blocksize, rounds):
        # Blok şifre çözme işlemini gerçekleştiren yardımcı metot
        w = blocksize // 2
        b = blocksize // 8
        mod = 2 ** w
        A = int.from_bytes(data[:b // 2], byteorder='little')
    ...
    def _encrypt_file(self, infile, outfile):
        # Dosya şifreleme işlemini gerçekleştiren metot
        w = self.blocksize // 2
        b = self.blocksize // 8
    ...
    def _decrypt_file(self, infile, outfile):
        # Dosya şifre çözme işlemini gerçekleştiren metot
        w = self.blocksize // 2
        b = self.blocksize // 8
        expanded_key = self._expand_key(self.key1, w, self.rounds)
        chunk = infile.read(b)
    ...
    def _encrypt_str(self, input_str):
        # Metni şifreleme işlemini gerçekleştiren metot
        str_in = BytesIO()
    ...
    def _decrypt_str(self, input_enc_str):
        # Şifreli metni çözme işlemini gerçekleştiren metot
    ...
    def _get_expanded_key(self):
        # Genişletilmiş anahtarı elde etme işlemini gerçekleştiren metot
        w = self.blocksize // 2
        expanded_key = self._expand_key(self.key1, w, self.rounds)
        return expanded_key

    def encrypt(self, infile, outfile):
        # Anahtarları kullanarak dosyayı şifreleyen metot
        w = self.blocksize // 2
        b = self.blocksize // 8
        ...
        while chunk:
            ...
            # Her anahtar için bloğu şifrele
    ...
    def decrypt(self, infile, outfile):
        # Anahtarları kullanarak dosyayı çözen metot
        w = self.blocksize // 2
        b = self.blocksize // 8
        expanded_key1 = self._expand_key(self.key1, w, self.rounds)
        expanded_key2 = self._expand_key(self.key2, w, self.rounds)
...
if __name__ == '__main__':
    ...
    # Kullanıcıdan şifrelenmek istenen metin alınır
    test_origin_char = input("Şifrelemek istediğiniz metni giriniz: ")

    # TripleDIS sınıfından bir örnek oluşturulur
